...
const load = (name, loaderTarget, loaderPosition = "beforeend") => {
    const _load = (name) => {
        ...
        item.promise = Promise.all([
            item.src && import(window._resolveImport?.(item.src) || item.src),
            loadStyle(item),
        ]);
        item.loaded = true;
    };
...
const processReveal = (name, settings) => {
...
const processEvent = (name, settings) => {
    const eventName = (settings.event || "").split("::")[0];
    ...
    const _handler = (event) => {
        ...
        const __dispatchEvent = async () => {
            const module = await import(
                window._resolveImport?.(settings.src) || settings.src
            );
...
const processPubSubEvent = (name, settings) => {
    if (!settings.PUB_SUB_EVENT) return;

    let unsub = subscribe(settings.PUB_SUB_EVENT, () => {
        load(name);
        unsub();
    });
};
...
const initialize = () => {
    ...
    Object.entries(deferredItems).forEach(([name, settings]) => {
        processReveal(name, settings);
        processEvent(name, settings);
        processPubSubEvent(name, settings);
    });
};
